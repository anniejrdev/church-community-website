...
export default function MinistryList() {
  ...
  // LOAD MINISTRIES FROM FIRESTORE
  useEffect(() => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "ministries", String(id)));
  ...
  const handleSaveAdd = async (newData) => {
    try {
      ...
      console.error(err);
      toast.error("Error adding ministry");
    }
  };
  ...
  const handleSaveEdit = async (updatedData) => {
    try {
      const docRef = doc(db, "ministries", String(updatedData.id));
      const docSnap = await getDoc(docRef);
      
      if (!docSnap.exists()) {
        toast.error("Document doesn't exist. Please refresh the page.");
        return;
      }
      ...
      console.error(err);
      toast.error("Error updating ministry");
    }
  };
